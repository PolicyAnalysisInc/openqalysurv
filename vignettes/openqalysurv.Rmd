...
vignette: >
  %\VignetteIndexEntry{openqalysurv}
...
Survival distributions can be modified using a variety of functions provided by openqalysurv. For example, the `apply_hr` function can be used to apply a hazard ratio to a survival distribution.
...
More complicated survival distributions can be created by stringing together multiple openqalysurv operations. Use of the `%>%` operator from magrittr is supported.
